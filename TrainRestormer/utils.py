...
def parse_args():
    ...
    parser.add_argument('--data_name', type=str, default='ASU_Turbulence_Clean_100k_dataset_v1', choices=['rain100L', 'rain100H', 'TurbRain', 'testTurb', 'ASU_Turbulence_Clean_100k_dataset_v1', 'MeanDistorted', 'Middle', 'MeanDistBlur'])
    ...
    parser.add_argument('--batch_size', nargs='+', type=int, default=[28, 20, 16, 8, 4, 4],
    ...
    parser.add_argument('--num_iter', type=int, default=200000, help='iterations of training')
    parser.add_argument('--model_file', type=str, default='', help='path of pre-trained model file')
...
class TurbDataset(Dataset):
    def __init__(self, data_path, data_name, data_type, patch_size=None, length=None):
        ...
        self.turb_images = sorted(glob.glob('{}/{}/{}/turb/*.png'.format(data_path, data_name, data_type)) + 
                                 glob.glob('{}/{}/{}/turb/*.jpg'.format(data_path, data_name, data_type)))
        self.noturb_images = sorted(glob.glob('{}/{}/{}/noturb/*.png'.format(data_path, data_name, data_type)) + 
                                   glob.glob('{}/{}/{}/noturb/*.jpg'.format(data_path, data_name, data_type)))


        # make sure the length of training and testing different
        self.num = len(self.turb_images)
    ...
    def __getitem__(self, idx):
        image_name = os.path.basename(self.turb_images[idx % self.num])
        turb = T.to_tensor(Image.open(self.turb_images[idx % self.num]))
        noturb = T.to_tensor(Image.open(self.noturb_images[idx % self.num]))
        h, w = turb.shape[1:]

        if self.data_type == 'train':
            # make sure the image could be cropped
            turb = pad_image_needed(turb, (self.patch_size, self.patch_size))
            noturb = pad_image_needed(noturb, (self.patch_size, self.patch_size))
            i, j, th, tw = RandomCrop.get_params(turb, (self.patch_size, self.patch_size))
            turb = T.crop(turb, i, j, th, tw)
            noturb = T.crop(noturb, i, j, th, tw)
            if torch.rand(1) < 0.5:
                turb = T.hflip(turb)
                noturb = T.hflip(noturb)
            if torch.rand(1) < 0.5:
                turb = T.vflip(turb)
                noturb = T.vflip(noturb)
        else:
            # padding in case images are not multiples of 8
            new_h, new_w = ((h + 8) // 8) * 8, ((w + 8) // 8) * 8
            pad_h = new_h - h if h % 8 != 0 else 0
            pad_w = new_w - w if w % 8 != 0 else 0
            turb = F.pad(turb, (0, pad_w, 0, pad_h), 'reflect')
            noturb = F.pad(noturb, (0, pad_w, 0, pad_h), 'reflect')
        return turb, noturb, image_name, h, w
