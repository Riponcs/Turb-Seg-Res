...
from utils import parse_args, TurbDataset, rgb_to_y, psnr, ssim
...
def test_loop(net, data_loader, num_iter):
    ...
    with torch.no_grad():
        ...
        for turb, noturb, name, h, w in test_bar:
            turb, noturb = turb.cuda(), noturb.cuda()
            out = torch.clamp((torch.clamp(model(turb)[:, :, :h, :w], 0, 1).mul(255)), 0, 255).byte()
            noturb = torch.clamp(noturb[:, :, :h, :w].mul(255), 0, 255).byte()
            # computer the metrics with Y channel and double precision
            y, gt = rgb_to_y(out.double()), rgb_to_y(noturb.double())
...
if __name__ == '__main__':
    args = parse_args()
    test_dataset = TurbDataset(args.data_path, args.data_name, 'test')
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=args.workers)

    print(args.data_name, args.data_path)
    ...
    if args.model_file:
        print('Saved Model detected, so testing the model')
        ...
        for n_iter in train_bar:
            ...
            if n_iter == 1 or n_iter - 1 in args.milestone:
                ...
                train_dataset = TurbDataset(args.data_path, args.data_name, 'train', args.patch_size[i], length)
                train_loader = iter(DataLoader(train_dataset, args.batch_size[i], True, num_workers=args.workers))
                i += 1
            # train
            model.train()
            turb, noturb, name, h, w = next(train_loader)
            turb, noturb = turb.cuda(), noturb.cuda()
            out = model(turb)
            loss = F.l1_loss(out, noturb)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            total_num += turb.size(0)
            total_loss += loss.item() * turb.size(0)
            train_bar.set_description('Train Iter: [{}/{}] Loss: {:.3f}'
                                      .format(n_iter, args.num_iter, total_loss / total_num))

            lr_scheduler.step()
            if n_iter % 8000 == 0 and n_iter!=0:
                print('Now it will test and Generate the Results'+str(n_iter))
